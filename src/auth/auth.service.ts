...
export class AuthService {
  ...
  async signupLocal(payload: TokenGenerationReq): Promise<ResponsePayLoad> {
    ...
    const account: AuthJwtDTO[] = await this.authRepo.verifyAccount(
      payload.username,
      payload.password,
    );
    if (account.length > 0) {
    ...
    const userTemp: AuthJwtDTO[] = await this.authRepo.verifyAccount(
      payload.username,
      payload.password,
    );
    const user = {
  ...
  async signinLocal(payload: TokenGenerationReq): Promise<ResponsePayLoad> {
    ...
    //check JWT
    const arrUser = await this.authRepo.verifyAccount(
      payload.username,
      payload.password,
    );
